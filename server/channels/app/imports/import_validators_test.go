...
func TestImportValidateUserImportData(t *testing.T) {
	...
	data.NotifyProps.Desktop = new(model.UserNotifyAll)
	...
	data.NotifyProps.Mobile = new(model.UserNotifyAll)
	data.NotifyProps.MobilePushStatus = new("invalid")
	checkError(t, ValidateUserImportData(&data))

	data.NotifyProps.MobilePushStatus = new(model.StatusOnline)
	...
	data.NotifyProps.CommentsTrigger = new(model.CommentsNotifyRoot)
...
func TestImportValidateUserAuth(t *testing.T) {
	tests := []struct {
		...
		{nil, new(""), true},
		{new(""), nil, true},
		{new(model.ServiceOpenid), new("foo@bar.baz"), true},
...
func TestValidateGuestRoles(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			input: UserImportData{
				Roles: new(model.SystemGuestRoleId),
				Teams: &[]UserTeamImportData{
					{
						Roles: new(model.TeamGuestRoleId),
						Channels: &[]UserChannelImportData{
							{Roles: new(model.ChannelGuestRoleId)},
						},
					},
				},
			},
		...
		},
		{
			...
			input: UserImportData{
				...
				Teams: &[]UserTeamImportData{
					...
					{
						Roles: new(model.TeamUserRoleId),
						Channels: &[]UserChannelImportData{
							{Roles: new(model.ChannelUserRoleId)},
						},
					},
				},
			},
		...
		},
		{
			...
			input: UserImportData{
				Roles: new(model.SystemUserRoleId),
				Teams: &[]UserTeamImportData{
					{
