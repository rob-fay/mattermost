	...
	testCases := []struct {
	...
	}{
		{
			...
			func(cfg *model.Config) {
				cfg.FileSettings.DriverName = new(model.ImageDriverLocal)
			},
		},
		{
			"s3",
			func(cfg *model.Config) {
				...
				cfg.FileSettings.DriverName = new(model.ImageDriverS3)
				cfg.FileSettings.AmazonS3AccessKeyId = new(model.MinioAccessKey)
				cfg.FileSettings.AmazonS3SecretAccessKey = new(model.MinioSecretKey)
				cfg.FileSettings.AmazonS3Bucket = new(model.MinioBucket)
