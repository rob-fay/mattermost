...
func TestOnConfigChange(t *testing.T) {
	t.Run("should switch between backends", func(t *testing.T) {
		...
		newConfig.ImageProxySettings.ImageProxyType = new(model.ImageProxyTypeLocal)
		...
		newConfig = proxy.ConfigService.Config().Clone()
		newConfig.ImageProxySettings.ImageProxyType = new(model.ImageProxyTypeAtmosCamo)
