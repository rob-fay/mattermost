...
func TestDesanitize(t *testing.T) {
	...
	target.LdapSettings.BindPassword = new(model.FakeSetting)
	target.FileSettings.PublicLinkSalt = new(model.FakeSetting)
	target.FileSettings.AmazonS3SecretAccessKey = new(model.FakeSetting)
	target.FileSettings.ExportAmazonS3SecretAccessKey = new(model.FakeSetting)
	target.EmailSettings.SMTPPassword = new(model.FakeSetting)
	target.GitLabSettings.Secret = new(model.FakeSetting)
	target.OpenIdSettings.Secret = new(model.FakeSetting)
	target.SqlSettings.DataSource = new(model.FakeSetting)
	target.SqlSettings.AtRestEncryptKey = new(model.FakeSetting)
	target.ElasticsearchSettings.Password = new(model.FakeSetting)
	target.ServiceSettings.GoogleDeveloperKey = new(model.FakeSetting)
	target.ServiceSettings.GiphySdkKey = new(model.FakeSetting)
